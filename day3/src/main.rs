...
mod tests {
    ...
    fn test_example() {
        let res =
            parse_muls("xmul(2,4)%&mul[3,7]!@^do_not_mul(5,5)+mul(32,64]then(mul(11,8)mul(8,5))");
        assert_eq!(res, vec![Mul(2, 4), Mul(5, 5), Mul(11, 8), Mul(8, 5)]);
    }

    #[test]
    fn test_example2() {
        let res =
            parse_muls("xmul(2,4)&mul[3,7]!^don't()_mul(5,5)+mul(32,64](mul(11,8)undo()?mul(8,5))");
        assert_eq!(
            res,
